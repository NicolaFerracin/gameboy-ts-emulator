...
export class PPU {
  ...
  constructor() {
    this._mem = new Uint8Array();
  }

  attachMemory(mem: Uint8Array) {
    this._mem = mem;
  }

  _isLCDOn = () => getBitAtPos(this._mem[NORMALIZED_LCDC_ADDR], 7) === 1;

  _getMode = () => {
    if (this.ly >= 144 && this.ly <= 153) return VBLANK_MODE;
  ...
  readByte(addr: u8) {
    ...
    return this._mem[addr - PPU_RESERVED_MEMORY_START];
  }

  writeByte(addr: u8, value: u8) {
    if (addr === STAT_ADDR) return this._setStat(value);
    else if (addr === LY_ADDR) this.ly = 0;
    else this._mem[addr - PPU_RESERVED_MEMORY_START] = value;
  }

  _getStat = () => {
    let stat = this._mem[NORMALIZED_STAT_ADDR];
    // update bits 0..1 with mode
    stat = setBitAtPos(stat, 0, getBitAtPos(this._mode, 0));
    stat = setBitAtPos(stat, 1, getBitAtPos(this._mode, 1));

    // update LYC==LY flag
    const lyc = this._mem[NORMALIZED_LYC_ADDR];
  ...
  _setStat = (value: u8) => {
    ...
    // bit 7 is unused
    this._mem[NORMALIZED_STAT_ADDR] = stat;
  };
